...
export default function ServiceReportsPage() {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      try {
        setLoading(true)
        const res = await fetch(`http://localhost:3005/api/reports/${categoryId}`)
        ...
        setServices(
          serviceId
            ? data.services.filter((s: any) => s.id === Number.parseInt(serviceId))
            : data.services
        )
  ...
  if (loading) {
    return <p className="text-center text-muted-foreground mt-10">Loading report data...</p>
  }
