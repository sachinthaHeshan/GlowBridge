...
export default function ServiceReportsPage() {
  ...
  if (minPrice) {
    services = services.filter((s) => s.finalPrice >= Number.parseFloat(minPrice))
  }
  if (maxPrice) {
    services = services.filter((s) => s.finalPrice <= Number.parseFloat(maxPrice))
  }
  ...
  const handleDownloadReport = () => {
    ...
    services.forEach((service) => {
      const serviceAppointments = reportAppointments.filter((apt) => apt.service_id === service.id)
      htmlContent += `
        <tr>
          <td>${service.name}</td>
          <td>${category?.name}</td>
    ...
    services.forEach((service) => {
      ...
      serviceAppointments.forEach((apt) => {
        const statusClass = apt.status === "completed" ? "status-completed" : "status-pending"
        htmlContent += `
          <tr>
            <td>${service.name}</td>
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push(`/dashboard/categories/${categoryId}/services`)}
          ...
          <p className="text-muted-foreground mt-2">Detailed report of services, pricing, and appointments</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <div className="text-sm text-muted-foreground flex items-center">Showing {services.length} service(s)</div>
          </div>
        </CardContent>
      </Card>
