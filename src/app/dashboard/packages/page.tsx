...
import { Plus, Edit, Trash2, Gift, Package, DollarSign, Users } from "lucide-react"
...
const mockPackages = [
  {
    ...
    services: ["Premium Haircut", "Hair Styling", "Bridal Makeup", "Luxury Manicure"],
    serviceCount: 4,
    totalPrice: 400,
    discount: 20,
    finalPrice: 320,
  ...
  },
  {
    ...
    services: ["Deep Tissue Massage", "Facial Treatment", "Aromatherapy", "Body Treatment"],
    serviceCount: 4,
    totalPrice: 350,
    discount: 15,
    finalPrice: 297.5,
  ...
  },
  {
    ...
    services: ["Premium Haircut", "Hair Coloring", "Hair Styling", "Hair Treatment"],
    serviceCount: 4,
    totalPrice: 320,
    discount: 25,
    finalPrice: 240,
...
export default function PackagesPage() {
  ...
  const [deleteConfirmation, setDeleteConfirmation] = useState({ isOpen: false, packageId: null })
  ...
  const handleSearch = (filters) => {
    ...
    if (filters.searchTerm) {
      filtered = filtered.filter(
        (pkg) =>
          ...
          pkg.description.toLowerCase().includes(filters.searchTerm.toLowerCase()) ||
          pkg.services.some((service) => service.toLowerCase().includes(filters.searchTerm.toLowerCase())),
      )
    }
    ...
    if (filters.priceRange && (filters.priceRange[0] > 0 || filters.priceRange[1] < 1000)) {
      filtered = filtered.filter(
        (pkg) => pkg.finalPrice >= filters.priceRange[0] && pkg.finalPrice <= filters.priceRange[1],
      )
    }
  ...
  const confirmDeletePackage = () => {
    if (deleteConfirmation.packageId) {
      const updatedPackages = packages.filter((pkg) => pkg.id !== deleteConfirmation.packageId)
      setPackages(updatedPackages)
      setFilteredPackages(updatedPackages)
    }
  ...
  const averageDiscount = packages.reduce((sum, pkg) => sum + pkg.discount, 0) / packages.length
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Create and manage service packages with special pricing</p>
        </div>
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <PackageForm onSubmit={handleAddPackage} onCancel={() => setIsAddDialogOpen(false)} />
          </DialogContent>
        </Dialog>
      </div>

      {/* Advanced Search */}
      <AdvancedSearch onSearch={handleSearch} onReset={handleResetSearch} type="packages" />
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-600">Showing {filteredPackages.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-r from-blue-50 to-blue-100 border-blue-200/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-blue-700">Avg. Services</p>
                <p className="text-2xl font-bold text-blue-800">
                  {Math.round(
                    filteredPackages.reduce((sum, p) => sum + p.serviceCount, 0) / filteredPackages.length || 0,
                  )}
                </p>
        ...
        <Card className="bg-gradient-to-r from-purple-50 to-purple-100 border-purple-200/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-purple-700">Avg. Discount</p>
                <p className="text-2xl font-bold text-purple-800">{averageDiscount.toFixed(0)}%</p>
          ...
          >
            <div className="relative overflow-hidden rounded-t-lg">
              ...
              <div className="absolute top-3 right-3 flex gap-2">
                {pkg.isPrivate && <Badge className="bg-purple-500 text-white text-xs">Private</Badge>}
                <Badge className="bg-green-500 text-white text-xs">{pkg.status}</Badge>
              </div>
              <div className="absolute bottom-3 left-3">
                <Badge className="bg-blue-500 text-white font-semibold">{pkg.discount}% OFF</Badge>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-blue-600">{pkg.serviceCount} Services</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <DollarSign className="w-4 h-4 text-green-500" />
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-green-600">${pkg.finalPrice.toFixed(2)}</span>
                      <span className="text-xs text-muted-foreground line-through">${pkg.totalPrice.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-green-700">Package Savings</span>
      ...
      {filteredPackages.length === 0 && (
        <Card className="p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-muted-foreground mb-2">No packages found</h3>
          <p className="text-sm text-muted-foreground">Try adjusting your search filters or create a new package.</p>
        </Card>
      )}
      ...
      <ConfirmationModal
        ...
        onClose={() => setDeleteConfirmation({ isOpen: false, packageId: null })}
